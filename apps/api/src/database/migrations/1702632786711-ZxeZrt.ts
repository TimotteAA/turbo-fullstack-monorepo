...
class ZxeZrt1702632786711 implements MigrationInterface {
    name = 'ZxeZrt1702632786711'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`rbac_resources\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL COMMENT '资源名称', \`label\` varchar(255) NULL COMMENT '资源显示名称', \`description\` varchar(255) NULL COMMENT '资源描述', \`type\` enum ('directory', 'menu', 'action') NOT NULL COMMENT '资源类型' DEFAULT 'menu', \`icon\` varchar(255) NULL COMMENT '用于侧边栏展示icon', \`path\` varchar(255) NULL COMMENT '菜单项路由路径', \`component\` varchar(255) NULL COMMENT '目录、菜单对应前端组件地址', \`status\` varchar(255) NOT NULL COMMENT '资源状态' DEFAULT 'enabled', \`external\` tinyint NULL COMMENT '是否外链' DEFAULT 0, \`show\` tinyint NULL COMMENT '是否显示在侧边栏中，用于目录或者菜单' DEFAULT 1, \`keepAlive\` tinyint NULL COMMENT '是否缓存，用于菜单项' DEFAULT 1, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`rule\` text NULL COMMENT '具体的权限规则', \`mpath\` varchar(255) NULL DEFAULT '', \`parentId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
...
module.exports = ZxeZrt1702632786711
